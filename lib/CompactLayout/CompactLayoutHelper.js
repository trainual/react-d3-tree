...
exports.calculateCompactFlexDimensions = function (root, compactLayout) {
    ...
    root.eachBefore(function (node) {
        ...
        if (node.children && node.children.length > 1) {
            ...
            var rowsMapNew = _this.groupBy(compactChildren_1, function (node) { return node.data.__rd3t.compact.row; }, function (reducedGroup) { return d3_1.max(reducedGroup, function (node) { return compactLayout.compactDimension.sizeRow(node) + compactLayout.compactMarginBetween(); }); });
...
exports.calculateCompactFlexPositions = function (root, compactLayout) {
    root.eachBefore(function (node) {
        if (node.children) {
            ...
            var rowsMapNew = _this.groupBy(compactChildren, function (node) { return node.data.__rd3t.compact.row; }, function (reducedGroup) { return d3_1.max(reducedGroup, function (node) { return compactLayout.compactDimension.sizeRow(node); }); });
