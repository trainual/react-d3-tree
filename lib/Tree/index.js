...
var LayoutDefaults_1 = require("../CompactLayout/LayoutDefaults");
var globalCss_1 = __importDefault(require("../globalCss"));
var CompactLayoutHelper_1 = require("../CompactLayout/CompactLayoutHelper");
var Tree = /** @class */ (function (_super) {
        ...
        _this.handleNodeToggle = function (nodeId) {
            ...
            if (_this.props.collapsible && !_this.state.isTransitioning) {
                if (targetNodeDatum.__rd3t.collapsed) {
                    ...
                    _this.props.shouldCollapseNeighborNodes && _this.collapseNeighborNodes(targetNodeDatum, data, _this.props.collapseChildrenOnToggle);
                }
                else {
                    Tree.collapseNode(targetNodeDatum, _this.props.collapseChildrenOnToggle);
                }
    ...
    Tree.assignInternalProperties = function (data, currentDepth) {
        ...
        return d.map(function (n) {
            var nodeDatum = n;
            nodeDatum.__rd3t = { id: null, depth: null, collapsed: false, compact: {} };
     ...
     * @static
     */
    Tree.collapseNode = function (nodeDatum, collapseChildrenOnToggle) {
     ...
     */
    Tree.prototype.collapseNeighborNodes = function (targetNode, nodeSet, collapseChildrenOnToggle) {
        ...
        neighbors.forEach(function (neighbor) { return Tree.collapseNode(neighbor, collapseChildrenOnToggle); });
    };
    /**
    ...
    Tree.prototype.generateTree = function () {
        var _this = this;
        var _a = this.props, initialDepth = _a.initialDepth, depthFactor = _a.depthFactor, separation = _a.separation, nodeSize = _a.nodeSize, orientation = _a.orientation, compact = _a.compact;
        var isInitialRenderForDataset = this.state.isInitialRenderForDataset;
        if (orientation === "vertical") {
            this.compactLayout = __assign(__assign({}, LayoutDefaults_1.VerticalDefaultLayout), this.props.compactLayout);
        }
        else {
            this.compactLayout = __assign(__assign({}, LayoutDefaults_1.HorizontalDefaultLayout), this.props.compactLayout);
        }
        var layout = d3_flextree_1.flextree({
            nodeSize: function (node) {
                var x = nodeSize.x, y = nodeSize.y;
                if (compact) {
                    return _this.compactLayout.nodeFlexSize({
                        compact: true,
                        node: node,
                        width: x,
                        height: y,
        ...
        var tree = layout.hierarchy(this.state.data[0], function (d) { return (d.__rd3t.collapsed ? null : d.children); });
        var rootNode = tree;
        rootNode.each(function (node, i, arr) {
            var x = nodeSize.x, y = nodeSize.y;
            Object.assign(node, { width: x, height: y });
        });
        if (compact) {
            CompactLayoutHelper_1.calculateCompactFlexDimensions(rootNode, this.compactLayout);
        }
        layout(tree);
        if (compact) {
            CompactLayoutHelper_1.calculateCompactFlexPositions(rootNode, this.compactLayout);
        }
